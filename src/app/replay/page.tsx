                  ...
                  {replayResult.normalEvents.map((event) => {
                    ...
                    return (
                      <div
                        key={event.eventId}
                        onClick={() => router.push(`/pulse/${encodeURIComponent(event.eventId)}`)}
                        className="card-glass card-glass-hover p-4 rounded-xl cursor-pointer transition-all"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-sm text-white">{event.symbol}</span>
                            <ConfidenceBadge level={event.confidence} />
                          </div>
                          <div className={`text-sm font-bold flex items-center gap-0.5 ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
                        ...
                        <div className="text-[11px] text-slate-500 flex items-center justify-between pt-2 border-t border-slate-800">
                          ...
                          <span>{event.unusualness > 0 ? `${event.unusualness}× normal` : 'Within normal bounds'}</span>
                        </div>
                      </div>
                    )
                  })}
